...
/**
 * catalog-service joins the event system — the last backend service to.
 *
 * Every other service acquired this wiring at the milestone that needed it:
 * cart in M7, pricing in M9, shipping in M10. Catalog never had a reason. M12
 * builds a search index **from catalog's events, never from its database**, so
 * now it does.
 *
 *   outbox            — used immediately: product.created / product.updated,
 *                       appended in the same transaction as the write.
 *   processed_events  — not used yet; catalog consumes nothing. It arrives
 *                       with its sibling because the two are one pattern,
 *                       exactly as pricing's outbox did in M9.
 */
export class CreateOutboxTables1735005100000 implements MigrationInterface {
